class UtilisateursController < ApplicationController
  skip_before_action :authorize, only: [:new, :create, :utilisateur_params]
  skip_before_action :authorizeAdmin, only: [:new, :create, :utilisateur_params]
  ...
  def create
    ...
    respond_to do |format|
      if @utilisateur.save
        format.html { redirect_to :controller => 'sessions', :action => 'new' , notice: 'Bienvenue', :nom_utilisateur => utilisateur_params[:nom_utilisateur], :password => utilisateur_params[:password]}
  ...
  private
    ...
    def utilisateur_params
      params.require(:utilisateur).permit(:nom_utilisateur, :password, :password_confirmation, :adresse_courriel, :est_auteur, :signature, :article)
    end
end
